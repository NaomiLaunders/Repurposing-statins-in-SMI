...
Trial1$MaxFU24<-as.integer(round(Trial1$MaxFU1*365.25))
Trial1$MaxFU24[Trial1$MaxFU24==0]<-1

Trial1$MaxFU12<-as.integer(round(Trial1$MaxFU1*365.25))
Trial1$MaxFU12[Trial1$MaxFU12>365]<-as.integer(365)
Trial1$MaxFU12[Trial1$MaxFU12==0]<-1

Trial1$MaxFU6<-as.integer(round(Trial1$MaxFU1*365.25))
Trial1$MaxFU6[Trial1$MaxFU6>182]<-as.integer(182)
Trial1$MaxFU6[Trial1$MaxFU6==0]<-1

Trial1$MaxFU3<-as.integer(round(Trial1$MaxFU1*365.25))
Trial1$MaxFU3[Trial1$MaxFU3>90]<-as.integer(90)
Trial1$MaxFU3[Trial1$MaxFU3==0]<-1

#In order as per protocol
ToImpute1<-dplyr::select(Trial1, patid, pracid, OutcomeAccidentStat12mo, OutcomePhysicalStat12mo, OutcomeCombSHStat12mo, OutcomeMHStat12mo,
                  ...
                  MaxFU24, Died2yr1, OutcomeAccidentStat24mo, OutcomePhysicalStat24mo, OutcomeCombSHStat24mo, OutcomeMHStat24mo) #outcomes
...
ImputedData1$method
save(ImputedData1, file="StatinCPRD/Data/ImputeTrial1_sens.Rdata")

####Mental health####
Pois<-glm(OutcomeMHStat12mo ~ offset(log(MaxFU12))+Active1, family=poisson, data=Trial1)

check_zeroinflation(Pois, tolerance = 0.05)
check_overdispersion(Pois)

#Is it still if use neg bin?
NegBin<-glm.nb(OutcomeMHStat12mo ~ offset(log(MaxFU12))+Active1, data=Trial1)
...
####Self harm####
#Dispersion
Pois<-glm(OutcomeCombSHStat12mo ~ offset(log(MaxFU12))+Active1, family=poisson, data=Trial1)

check_zeroinflation(Pois, tolerance = 0.05)
check_overdispersion(Pois)

#Is it still if use neg bin?
NegBin<-glm.nb(OutcomeCombSHStat12mo ~ offset(log(MaxFU12))+Active1, data=Trial1)
...
####Physical####
#dispersion
Pois<-glm(OutcomePhysicalStat12mo ~ offset(log(MaxFU12))+Active1, family=poisson, data=Trial1)

check_zeroinflation(Pois, tolerance = 0.05)
check_overdispersion(Pois)

#Is it still if use neg bin?
NegBin<-glm.nb(OutcomePhysicalStat12mo ~ offset(log(MaxFU12))+Active1, data=Trial1)
...
####Accident####
#dispersion
Pois<-glm(OutcomeAccidentStat12mo ~ offset(log(MaxFU12))+Active1, family=poisson, data=Trial1)

check_zeroinflation(Pois, tolerance = 0.05)
check_overdispersion(Pois)

#Is it still if use neg bin?
NegBin<-glm.nb(OutcomeAccidentStat12mo ~ offset(log(MaxFU12))+Active1, data=Trial1)
...
####Final models####
#Undjusted
Unadj <- with(data=ImputedData1, glm.nb(OutcomeCombSHStat12mo ~ offset(log(MaxFU12))+Active1))
...
for (i in (1:1)) {
  Unadj <- lapply(datlist, FUN=function(data){
    glm.nb(as.formula(paste0(Outcome[i], "~ offset(log(", Time[i], ")) +Active1")), data=data)
  })
...
for (i in (1:1)) {
  Adj <- lapply(datlist, FUN=function(data){
    glm.nb(as.formula(paste0(Outcome[i], "~ offset(log(", Time[i], ")) +Active1+AgeTrial1+YearStat+SMIStatTime+DislipTimeStat+StatDose1+gender+
                  ...
                  APStatTrial1+PriorPhysicalBinStat+PriorAccidentBinStat+FullIMD")), control = glm.control(maxit = 50), data=data)
  })
...
TimePoint<-c(replicate(240,"3"),replicate(240,"6"),replicate(240,"24"),replicate(240,"12"))
Group<-c(replicate(60,"Accident"), replicate(60, "Phys"),replicate(60,"SH"),replicate(60,"MH"))
Group<-c(replicate(4, Group))
...
write.csv(UnadjResult, "StatinCPRD/Outputs/Trial1UnadjSens.csv")
write.csv(AdjResults, "StatinCPRD/Outputs/Trial1AdjSens.csv")
