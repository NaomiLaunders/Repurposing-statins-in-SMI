...
#TRIAL 2: Simvastatin/atorvastatin to any other statin; prav or rosuv to any other statin; risperidone, aripiprazole, olanzapine to any other.

####Trial 2####
#AP switching
PatAPAll<-subset(PatAPAll, patid %in% Trial2$patid)
...
Trial2AP<-subset(AP, !is.na(Trial2StatDate) & eventdate-Trial2StatDate>=-182.625 & eventdate<=EndDate1)

#Discontinued antipsychotic: Gap of more than 90 days
Trial2Discont<-subset(Trial2AP, AP=="Aripiprazole" | AP=="Risperidone" | AP=="Olanzapine")

FirstStudyAP<-Trial2Discont%>%
  subset(eventdate-Trial2StatDate<=0)%>%
  group_by(patid)%>%
...
Trial2Discont<-merge(x=Trial2Discont, y=FirstStudyAP, by="patid", all.x=TRUE, all.y=TRUE)

Trial2Discont<-Trial2Discont%>%
...
length(unique(Trial2Discont$patid))

#Switched statin:Treatment: First non-simvastatin/atorvastatin prescription
Trial2Stat<-subset(TrialStat, !is.na(Trial2StatDate))
Trial2Stat<-subset(Trial2Stat, issuedate<=EndDate1 & issuedate-Trial2StatDate>=0)

#First switch
T1AS<-subset(Trial2Stat, Statin!="Simvastatin" & Statin!="Atorvastatin" & Active2==1)
T1AS<-T1AS%>%
  group_by(patid)%>%
...
#Switched statin:Comparator: First non-pravastatin
T1CS<-subset(Trial2Stat, Statin!="Pravastatin" & Active2==0)
T1CS<-T1CS%>%
  group_by(patid)%>%
  mutate(FirstSwitch=min(issuedate))%>%
  subset(FirstSwitch==issuedate)%>%
...
PPA<-merge(x=PPA, y=Trial2Discont, by="patid", all.x=TRUE, all.y=TRUE)

PPA$PPADate<-pmin(PPA$Discont, PPA$DiscontStat, PPA$FirstSwitch,na.rm=TRUE)
PPA<-select(PPA, patid, PPADate)

#####Now bind back on to trial####
Trial2<-merge(x=Trial2, y=PPA, by="patid", all.x=TRUE, all.y=TRUE)
Trial2$Yr2_1<-as.Date(Trial2$Trial2StatDate+730.5)

Trial2$PPAEnd<-pmin(Trial2$end, Trial2$Yr2_1, Trial2$PPADate, na.rm=TRUE)
Trial2$PPAFU<-as.numeric(Trial2$PPAEnd-Trial2$Trial2StatDate)
summary(Trial2$PPAFU)
...
NewFields<-select(Trial2, patid, MHBin24, MHBin12,MHBin6,MHBin3,SHBin24,SHBin12,SHBin6,SHBin3,
...
datlist<-lapply(datlist, FUN=function(Data){
  Data<-Data%>%
    mutate(APStatTrial2=relevel(APStatTrial2, ref="olanzapine"))
})
#Do the first one to create data
for (i in (1:1)) {
...
write.csv(UnadjResult, "StatinCPRD/Outputs/Trial2Unadj_PPA.csv")
write.csv(AdjResult, "StatinCPRD/Outputs/Trial2Adj_PPA.csv")   
